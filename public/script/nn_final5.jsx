...
// ═══ POSITION ALL NODES ═══
(function(){
  // Y ranges — precise spacing: nodeH=14, gap=6, spacing=20px per node
  // A: 22 S0 dots(264px) + 10 S1 enc(200px) → use 85-350
  // B: 8 S1 enc(160px) → use 360-520
  // C: 6 S1 enc(120px) → use 530-650
  const MY={A:[85,350],B:[360,520],C:[530,650]};
  // S3 AB: 13 nodes(260px) → 85-345
  // S3 AC: 2 nodes(40px) → 360-400
  // S3 BC: 5 nodes(100px) → 415-560
  const CROSS_Y={AB:[85,345],AC:[360,400],BC:[415,560],ABC:[250,450]};
  ...
  // Repr nodes — centered in each modality band
  const f=id=>N.find(n=>n.id===id);
  f("hA").x=SX[2];f("hA").y=215;
  f("hB").x=SX[2];f("hB").y=440;
  f("hC").x=SX[2];f("hC").y=590;

  // Struct nodes
  f("st-coAB").x=SX[2.5];f("st-coAB").y=310;
  f("st-coBC").x=SX[2.5];f("st-coBC").y=490;
  f("st-coAC").x=SX[2.5];f("st-coAC").y=400;
  f("st-cat").x=SX[3.5];f("st-cat").y=400;

  // S4: courses on TOP, structural below
  const s4Courses=N.filter(n=>n.st===4&&!n.type);
  s4Courses.sort((a,b)=>a.tOrd-b.tOrd);
  s4Courses.forEach((n,i)=>{n.x=SX[4];n.y=270+i*45});
  // Diploma=270, MLAlgo=315, PhysML=360, MediaTech=405
  f("st-sa").x=SX[4];f("st-sa").y=490;
  f("st-ffn").x=SX[4]+5;f("st-ffn").y=540;
  f("st-an").x=SX[4.5];f("st-an").y=515;
...
function NNSelfAttn(){
  ...
  useEffect(()=>{
    ...
    function draw(){
      ...
      // ═══ MODALITY BANDS ═══
      [{y:85,h:270,c:MC.A,l:"A · Art & Design"},{y:358,h:165,c:MC.B,l:"B · Tech & HCI"},{y:528,h:125,c:MC.C,l:"C · Econ & Compute"}].forEach(b=>{
      ...
      ctx.fillStyle="rgba(215,195,140,0.008)";ctx.fillRect(30,25,SX[6]-60,55);
      ctx.save();
      ...
      SELF_ATT.forEach(([a,b,w])=>{
        ...
        else if(both){ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},${0.3+w*0.5})`;ctx.lineWidth=0.5+w*1.2}
        else if(one){ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},${0.06+w*0.15})`;ctx.lineWidth=0.15+w*0.4}
      ...
      E.forEach(([a,b,ew])=>{
        ...
        if(ha){ctx.strokeStyle=ia?`rgba(${cc.r},${cc.g},${cc.b},${0.15+w*0.5})`:"rgba(255,255,255,0.002)";ctx.lineWidth=ia?(0.4+w*0.8):0.02}
      ...
      ctx.fillText("residual skip (h_A)",830,95);ctx.fillStyle="rgba(155,145,185,0.3)";ctx.fillText("residual skip (h_C)",830,640);ctx.restore();
