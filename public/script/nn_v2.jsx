const { useState, useRef, useEffect, useCallback } = React;

const MC={A:{r:200,g:170,b:120},B:{r:130,g:175,b:155},C:{r:155,g:145,b:185},AB:{r:170,g:172,b:135},AC:{r:178,g:158,b:152},BC:{r:142,g:160,b:170},ABC:{r:210,g:195,b:145},F:{r:215,g:195,b:140},T:{r:175,g:170,b:162}};
const SA_C={THU:null,Cornell:{r:180,g:40,b:40},Harvard:{r:200,g:170,b:100},MIT:{r:100,g:180,b:200}};

// ═══ LAYOUT CONSTANTS ═══
// Wide canvas, generous spacing
const CW=1500, CH=700;

// Time → X: each year gets ~180px
const TX={
  "20a":80,"20b":130,"21a":210,"21b":280,
  "22a":370,"22b":440,"23f":530,"24s":620,
  "24b":690,"25a":770,"25b":830,
  "25f":910,"26s":1010,
  // Structural
  "repr":860,"cotrm":900,"cat":1080,"trm":1140,"an":1200,
  "skill":1380,
};

// Modality Y bands — CLEAR SEPARATION (60px gaps)
// Band A: y 50–200 (Art)
// Band B: y 260–380 (Tech)  
// Band C: y 440–540 (Econ)
// Projects: y 580–660
// Skills: full height at far right
function bandY(mod,i,n){
  const B={A:[55,195],B:[265,375],C:[445,535]};
  const b=B[mod]||B.A;
  return n<=1?(b[0]+b[1])/2:b[0]+(b[1]-b[0])*i/(n-1);
}

// ═══ NODES — organized by time, no overlapping ═══
const N=[];
function add(id,l,tm,st,mod,sch,extra){N.push({id,l,tm,st,mod,sch,...extra})}

// ─── 2020-S1: Art foundation (6 courses) ───
add("a01","Form Study","20a",0,"A","THU");
add("a02","Visual Lang I","20a",0,"A","THU");
add("a03","Visual Lang II","20a",0,"A","THU");
add("a04","3D Fundmntl","20a",0,"A","THU");
add("a05","CN Art Hist","20a",0,"A","THU");
add("a06","Arts&Crafts","20a",0,"A","THU");

// ─── 2020-S2 (5) ───
add("a07","Calligraphy","20b",0,"A","THU");
add("a08","Foreign Art","20b",0,"A","THU");
add("a09","Western Dsgn","20b",0,"A","THU");
add("a10","Color Paint","20b",0,"A","THU");
add("a11","Forming I","20b",0,"A","THU");

// ─── 2021-S1: Textile (6) ───
add("a12","Seal Carving","21a",0,"A","THU");
add("a13","Dye Pattern","21a",0,"A","THU");
add("a14","Color Textile","21a",0,"A","THU");
add("a15","Textile Hist","21a",0,"A","THU");
add("a16","Textile Mat","21a",0,"A","THU");
add("a17","Print&Dye I","21a",0,"A","THU");

// ─── 2021-S2 (5) ───
add("a18","Logo Design","21b",0,"A","THU");
add("a19","Embroidery","21b",0,"A","THU");
add("a20","Weaving Tech","21b",0,"A","THU");
add("a21","Print&Dye II","21b",0,"A","THU");
add("a22","Weaving","21b",0,"A","THU");

// ─── 2022-S1: Art enc + NEW Econ tokens ───
add("e-arch","Cultural Arch","22a",1,"A","THU");
add("e-video","Video&Audio","22a",1,"A","THU");
add("e-creative","Creative Think","22a",1,"A","THU");
add("e-film","Language Film","22a",1,"A","THU");
// Econ enters
add("c01","Accounting","22a",0,"C","THU");
add("c02","Econ I","22a",0,"C","THU");
// First cross-modal
add("x01","Dynamic Infogfx","22a",3,"AB","THU");
add("x02","InfoDesign&Art","22a",3,"AB","THU");

// ─── 2022-S2 ───
add("c03","Econ II","22b",0,"C","THU");
add("c04","China Econ","22b",0,"C","THU");
add("e-exhib","Exhibition","22b",1,"A","THU");
add("e-newmed","New Media Art","22b",1,"A","THU");
add("x03","Prototyping","22b",3,"AB","THU");
add("x04","UI Foundation","22b",3,"AB","THU");
add("x14","Info Gfx Dsgn","22b",3,"AC","THU");

// ─── 2023-F: Cornell — NEW B tokens + C enc ───
add("b01","Intro Computing","23f",0,"B","Cornell");
add("b02","Dsgn&Prog Web","23f",0,"B","Cornell");
add("c05","Financial Rpt","23f",1,"C","Cornell");
add("x05","HCI Studio","23f",3,"AB","Cornell");

// ─── 2024-S: Cornell spring ───
add("x06","IxD Studio","24s",3,"AB","Cornell");
add("x07","Intermed Web","24s",3,"AB","Cornell");
add("x16","Intro DataSci","24s",3,"BC","Cornell");

// ─── 2024-S2: THU Tech encoders ───
add("e-iface","Interface Dsgn","24b",1,"B","THU");
add("e-ixt1","Ix Tech I","24b",1,"B","THU");
add("e-web","Design Web","24b",1,"B","THU");
add("e-smart","Smart Space","24b",1,"B","THU");
add("e-ixt2","Ix Tech II","24b",1,"B","THU");
add("e-usab","Usability","24b",1,"B","THU");
add("e-infod1","InfoDsgn AIGC","24b",1,"AB","THU");
add("e-method","Methodology","24b",1,"A","THU");
add("e-dsgnpsy","Design Psych","24b",1,"A","THU");
add("x08","Ix Design I","24b",3,"AB","THU");

// ─── 2025-S1: Advanced everything ───
add("x09","Comp Train II","25a",3,"AB","THU");
add("x10","Prof Practice","25a",3,"AB","THU");
add("e-ubiq","Ubiquitous Tech","25a",1,"B","THU");
add("e-display","Display Dsgn","25a",1,"A","THU");
add("x11","Ix Design II","25a",3,"AB","THU");
add("x12","InfoDsgn TD","25a",3,"AB","THU");
add("e-micro","Micro Econ","25a",1,"C","THU");
add("e-macro","Macro Econ","25a",1,"C","THU");
add("e-corpfin","Corp Finance","25a",1,"C","THU");
add("e-invest","Investment","25b",1,"C","THU");
add("e-cpp","C++ Prog","25b",1,"C","THU");
add("x17","Cog Psychology","25b",3,"BC","THU");
add("t-diploma","Diploma Thesis","25b",4,"ABC","THU");

// ─── 2025-F: Harvard ───
add("e-prosem","Proseminar","25f",1,"A","Harvard");
add("x13","Digital Prod","25f",3,"AB","Harvard");
add("x15","Quant Aesthetics","25f",3,"AC","Harvard");
add("x18","Adv DataSci","25f",3,"BC","Harvard");

// ─── 2026-S: MIT ───
add("x19","Biomech Move","26s",3,"BC","Harvard");
add("x20","Mobile Sensor","26s",3,"BC","MIT");
add("t-mlalgo","ML Algo→Apps","26s",4,"ABC","MIT");
add("t-physml","Physics ML","26s",4,"ABC","MIT");
add("t-media","Media Tech","26s",4,"ABC","MIT");

// ═══ PROJECTS — in dedicated row below modality bands ═══
// Positioned at their real time, y=590-660
add("o-tide","TideEcho","22b",5,"AB","THU",{yr:2022});
add("o-seepal","SeePal","23f",5,"AB","THU",{yr:2023});
add("o-shadow","ShadowPlay","24s",5,"AB","THU",{yr:2024});
add("o-ehoura","Ehoura","24b",5,"AB","THU",{yr:2024});
add("o-seren","SerenEcho","25a",5,"AB","THU",{yr:2025});
add("o-symbio","Symbiophony","25a",5,"AB","THU",{yr:2025});
add("o-tuch","Tuchsure","25b",5,"ABC","THU",{yr:2025});
add("o-skin","SkinMe","25b",5,"BC","THU",{yr:2025});
add("o-light","LightScale","25b",5,"A","THU",{yr:2025});
add("o-pgmoe","PG-MoE","26s",5,"BC","Harvard",{yr:2026});
add("o-audeate","Audeate","26s",5,"ABC","Harvard",{yr:2026});

// ═══ STRUCTURAL ═══
add("hA","h_A","repr",2,"A",null,{type:"repr"});
add("hB","h_B","repr",2,"B",null,{type:"repr"});
add("hC","h_C","repr",2,"C",null,{type:"repr"});
add("st-coAB","Co-TRM A↔B","cotrm",2.5,"AB",null,{type:"struct"});
add("st-coBC","Co-TRM B↔C","cotrm",2.5,"BC",null,{type:"struct"});
add("st-coAC","Co-TRM A↔C","cotrm",2.5,"AC",null,{type:"struct"});
add("st-cat","Concat+Proj","cat",3.5,"F",null,{type:"struct"});
add("st-sa","Self-Attention","trm",4,"T",null,{type:"struct"});
add("st-ffn","FFN","trm",4,"T",null,{type:"struct"});
add("st-an","Add & Norm","an",4,"T",null,{type:"struct"});

// ═══ SKILLS ═══
const SKILLS=["Multimodal AI","ML Systems","Edge AI","Embedded Sensing","Physical Comp",
  "Full-Stack Proto","Human-AI Ix","Interaction Dsgn","Tangible Interface","Info Viz",
  "UX Research","Data-Driven Dsgn","Quant Analysis","System Thinking","Wearable Proto",
  "Research→Proto","Creative Coding","Sensor Fusion"];
const skMods=["ABC","BC","BC","BC","AB","ABC","AB","AB","AB","AC","AB","AC","C","ABC","AB","ABC","AB","BC"];
SKILLS.forEach((s,i)=>add("s"+String(i).padStart(2,"0"),s,"skill",6,skMods[i],null));

// ═══ POSITION ALL NODES ═══
(function(){
  // Group by tm+mod for Y distribution
  const g={};
  N.forEach(n=>{
    // Projects get special Y
    if(n.st===5){n.x=TX[n.tm]||600;n.y=595+Math.random()*40;return}
    // Skills get special Y  
    if(n.st===6){n.x=TX.skill;return}
    const k=n.tm+"|"+(n.mod.length>1?"X":n.mod);
    (g[k]||(g[k]=[])).push(n);
  });
  // Position grouped nodes
  Object.values(g).forEach(arr=>{
    arr.forEach((n,i)=>{
      n.x=TX[n.tm]||500;
      if(n.type==="repr"||n.type==="struct"){
        // Already has manual y below
      }else{
        const baseMod=n.mod[0];
        n.y=bandY(baseMod,i,arr.length);
      }
    });
  });
  // Skills evenly distributed
  const sk=N.filter(n=>n.st===6);
  sk.forEach((s,i)=>{s.x=TX.skill;s.y=50+i*(CH-100)/(sk.length-1)});
  // Fix project Y: distribute evenly in project band
  const projs=N.filter(n=>n.st===5);
  projs.sort((a,b)=>(TX[a.tm]||0)-(TX[b.tm]||0));
  projs.forEach((p,i)=>{p.y=590+(i%2)*35});
  // Fix structural Y
  const f=id=>N.find(n=>n.id===id);
  if(f("hA")){f("hA").y=125}
  if(f("hB")){f("hB").y=320}
  if(f("hC")){f("hC").y=490}
  if(f("st-coAB")){f("st-coAB").y=225}
  if(f("st-coBC")){f("st-coBC").y=420}
  if(f("st-coAC")){f("st-coAC").y=325}
  if(f("st-cat")){f("st-cat").y=320}
  if(f("st-sa")){f("st-sa").y=280}
  if(f("st-ffn")){f("st-ffn").y=340}
  if(f("st-an")){f("st-an").y=395}
})();

const nMap=Object.fromEntries(N.map(n=>[n.id,n]));

// ═══ SELF-ATTENTION ═══
const SELF_ATT=[
  ["a01","a02",0.8],["a02","a03",0.9],["a01","a04",0.7],["a10","a14",0.8],
  ["a07","a12",0.85],["a13","a17",0.9],["a20","a22",0.9],["a19","a20",0.7],
  ["a16","a15",0.75],["a05","a08",0.8],["a06","a09",0.8],["a11","a04",0.7],
  ["b01","b02",0.9],["c01","c02",0.7],["c02","c03",0.95],["c03","c04",0.6],
  ["e-video","e-film",0.9],["e-exhib","e-display",0.85],["e-creative","e-newmed",0.6],
  ["e-infod1","e-method",0.8],["e-ixt1","e-ixt2",0.95],["e-iface","e-web",0.8],
  ["e-smart","e-ubiq",0.85],["e-usab","e-iface",0.7],["e-ixt2","e-ubiq",0.8],
  ["e-micro","e-macro",0.9],["e-corpfin","e-invest",0.85],
  ["x01","x02",0.85],["x03","x04",0.8],["x05","x06",0.9],["x08","x11",0.95],
  ["x11","x12",0.75],["x14","x15",0.7],["x16","x18",0.85],["x18","x20",0.7],
  ["x19","x20",0.75],["t-mlalgo","t-physml",0.9],["t-mlalgo","t-media",0.7],
];
const SA_W={};SELF_ATT.forEach(([a,b,w])=>{SA_W[a+"↔"+b]=w;SA_W[b+"↔"+a]=w});

// ═══ EDGES ═══
const E=[];
let _s=42;function sr(){_s=((_s*1103515245+12345)&0x7fffffff);return _s/0x7fffffff}

// S0→S1
N.filter(n=>n.st===0).forEach(t=>{
  const tg=N.filter(e=>e.st===1&&(e.mod===t.mod||e.mod.includes(t.mod))&&(TX[e.tm]||0)>=(TX[t.tm]||0));
  [...tg].sort(()=>sr()-0.5).slice(0,Math.min(2,tg.length)).forEach(e=>E.push([t.id,e.id]));
});
// S1→repr
N.filter(n=>n.st===1).forEach(e=>{
  if(e.mod==="A"||e.mod==="AB")E.push([e.id,"hA"]);
  if(e.mod==="B"||e.mod==="AB")E.push([e.id,"hB"]);
  if(e.mod==="C")E.push([e.id,"hC"]);
});
E.push(["hA","st-coAB"],["hB","st-coAB"],["hB","st-coBC"],["hC","st-coBC"],["hA","st-coAC"],["hC","st-coAC"]);
N.filter(n=>n.st===3&&n.mod==="AB").forEach(n=>E.push(["st-coAB",n.id]));
N.filter(n=>n.st===3&&n.mod==="BC").forEach(n=>E.push(["st-coBC",n.id]));
N.filter(n=>n.st===3&&n.mod==="AC").forEach(n=>E.push(["st-coAC",n.id]));
N.filter(n=>n.st===3).forEach(n=>E.push([n.id,"st-cat"]));
E.push(["st-cat","st-sa"],["st-sa","st-ffn"],["st-ffn","st-an"]);
N.filter(n=>n.st===4&&!n.type).forEach(n=>{E.push(["st-cat",n.id]);E.push([n.id,"st-an"])});
E.push(["hA","st-an"],["hC","st-an"],["st-cat","st-an"]);
// Late outputs from addnorm
N.filter(n=>n.st===5&&(n.yr||0)>=2025).forEach(n=>E.push(["st-an",n.id]));
// Early outputs from nearby courses
N.filter(n=>n.st===5&&(n.yr||0)<2025).forEach(o=>{
  const near=N.filter(n=>(n.st===1||n.st===3)&&Math.abs((TX[n.tm]||0)-(TX[o.tm]||0))<100&&[...o.mod].some(m=>n.mod.includes(m)));
  near.sort(()=>sr()-0.5).slice(0,3).forEach(n=>E.push([n.id,o.id]));
});
// S5→S6
N.filter(n=>n.st===5).forEach(o=>{
  N.filter(s=>s.st===6&&[...o.mod].some(m=>s.mod.includes(m))).sort(()=>sr()-0.5).slice(0,3).forEach(s=>E.push([o.id,s.id]));
});
// Long skips
["e-ubiq","e-ixt2","e-prosem","e-cpp"].forEach(id=>{
  N.filter(n=>n.st===4&&!n.type).forEach(t=>E.push([id,t.id]));
});

const skipSet=new Set(["hA→st-an","hC→st-an","st-cat→st-an",...["e-ubiq","e-ixt2","e-prosem","e-cpp"].flatMap(id=>N.filter(n=>n.st===4&&!n.type).map(t=>id+"→"+t.id))]);

function trace(id){
  const fw={},bw={};
  E.forEach(([a,b])=>{(fw[a]||(fw[a]=[])).push(b);(bw[b]||(bw[b]=[])).push(a)});
  const ns=new Set(),es=new Set();
  const f=i=>{if(ns.has(i))return;ns.add(i);(fw[i]||[]).forEach(n=>{es.add(i+"→"+n);f(n)})};
  const b=i=>{if(ns.has(i))return;ns.add(i);(bw[i]||[]).forEach(n=>{es.add(n+"→"+i);b(n)})};
  b(id);ns.delete(id);f(id);
  const saNeighbors={};
  SELF_ATT.forEach(([a,b,w])=>{
    if(ns.has(a)||a===id){saNeighbors[b]=Math.max(saNeighbors[b]||0,w)}
    if(ns.has(b)||b===id){saNeighbors[a]=Math.max(saNeighbors[a]||0,w)}
  });
  return{ns,es,saNeighbors};
}

function NNSelfAttn(){
  const cvs=useRef(null);const[act,setAct]=useState(null);const[hov,setHov]=useState(null);const[tip,setTip]=useState(null);
  const pts=useRef([]);const raf=useRef(null);const td=useRef({ns:new Set(),es:new Set(),saNeighbors:{}});const amb=useRef(null);const tm2=useRef(0);
  const dpr=typeof window!=='undefined'?(window.devicePixelRatio||1):1;

  const spawn=useCallback(ed=>{const r=[];ed.forEach(e=>{const[a,b]=e.split("→");const f=nMap[a],t=nMap[b];if(f&&t)for(let i=0;i<2;i++)r.push({fx:f.x,fy:f.y,tx:t.x,ty:t.y,t:Math.random()*0.3,sp:0.002+Math.random()*0.003,sz:0.4+Math.random()*1,a:true})});pts.current=r},[]);
  const hit=useCallback((mx,my)=>{for(const n of N){if(!n.x)return null;const dx=mx-n.x,dy=my-n.y;const r=n.type==="struct"?22:n.type==="repr"?16:n.st===0?6:n.st===6?14:n.st===5?12:10;if(dx*dx+dy*dy<(r+5)*(r+5))return n}return null},[]);
  const oc=useCallback(e=>{const r=cvs.current.getBoundingClientRect();const n=hit((e.clientX-r.left)*(CW/r.width),(e.clientY-r.top)*(CH/r.height));if(n&&n.id!==act){setAct(n.id);const d=trace(n.id);td.current=d;spawn(d.es)}else{setAct(null);td.current={ns:new Set(),es:new Set(),saNeighbors:{}};pts.current=[]}},[act,spawn,hit]);
  const om=useCallback(e=>{const r=cvs.current.getBoundingClientRect();const n=hit((e.clientX-r.left)*(CW/r.width),(e.clientY-r.top)*(CH/r.height));setHov(n?n.id:null);setTip(n?{x:e.clientX-r.left+12,y:e.clientY-r.top-30,n}:null);cvs.current.style.cursor=n?'pointer':'default'},[hit]);

  useEffect(()=>{
    const c=cvs.current,ctx=c.getContext("2d");c.width=CW*dpr;c.height=CH*dpr;ctx.scale(dpr,dpr);
    if(!amb.current){amb.current=[];for(let i=0;i<160;i++)amb.current.push({x:Math.random()*CW,y:Math.random()*CH,vx:(Math.random()-0.5)*0.04,vy:(Math.random()-0.5)*0.02,sz:Math.random()*0.7,al:0.03+Math.random()*0.1})}
    function rr(x,y,w,h,r){ctx.beginPath();ctx.moveTo(x+r,y);ctx.arcTo(x+w,y,x+w,y+h,r);ctx.arcTo(x+w,y+h,x,y+h,r);ctx.arcTo(x,y+h,x,y,r);ctx.arcTo(x,y,x+w,y,r);ctx.closePath()}

    function draw(){
      tm2.current+=0.016;ctx.fillStyle="#0b0b0b";ctx.fillRect(0,0,CW,CH);
      const ha=act!==null;const{ns:aN,es:aE,saNeighbors}=td.current;

      // Grid
      ctx.fillStyle="rgba(255,255,255,0.005)";for(let x=10;x<CW;x+=20)for(let y=10;y<CH;y+=20){ctx.beginPath();ctx.arc(x,y,0.2,0,Math.PI*2);ctx.fill()}

      // Ambient
      if(!ha)amb.current.forEach(p=>{p.x+=p.vx;p.y+=p.vy;if(p.x<0)p.x=CW;if(p.x>CW)p.x=0;if(p.y<0)p.y=CH;if(p.y>CH)p.y=0;ctx.beginPath();ctx.arc(p.x,p.y,p.sz,0,Math.PI*2);ctx.fillStyle=`rgba(190,175,130,${p.al})`;ctx.fill()});

      // ═══ TIME AXIS ═══
      ctx.save();ctx.font="500 9px 'SF Mono','Menlo',monospace";ctx.textAlign="center";
      [{x:80,t:"2020"},{x:210,t:"2021"},{x:370,t:"2022"},{x:530,t:"2023"},{x:650,t:"2024"},{x:800,t:"2025"},{x:1010,t:"2026"},{x:1200,t:"→"},{x:1380,t:"Skills"}].forEach(lb=>{
        ctx.fillStyle="rgba(255,255,255,0.35)";ctx.fillText(lb.t,lb.x,22);
      });
      ctx.strokeStyle="rgba(255,255,255,0.06)";ctx.lineWidth=0.5;
      ctx.beginPath();ctx.moveTo(50,30);ctx.lineTo(1320,30);ctx.stroke();
      [80,210,370,530,650,800,1010].forEach(x=>{ctx.beginPath();ctx.moveTo(x,27);ctx.lineTo(x,33);ctx.stroke()});
      ctx.restore();

      // ═══ MODALITY BANDS ═══
      [{y:40,h:170,c:MC.A,l:"A · Art & Design"},{y:250,h:140,c:MC.B,l:"B · Tech & HCI"},{y:430,h:120,c:MC.C,l:"C · Econ & Compute"}].forEach(b=>{
        ctx.fillStyle=`rgba(${b.c.r},${b.c.g},${b.c.b},0.008)`;ctx.fillRect(40,b.y,CW-80,b.h);
        ctx.save();ctx.font="500 6px 'SF Mono',monospace";ctx.fillStyle=`rgba(${b.c.r},${b.c.g},${b.c.b},0.15)`;
        ctx.textAlign="left";ctx.fillText(b.l,45,b.y+12);ctx.restore();
      });
      // Project band
      ctx.fillStyle="rgba(215,195,140,0.005)";ctx.fillRect(40,575,CW-80,80);
      ctx.save();ctx.font="500 6px 'SF Mono',monospace";ctx.fillStyle="rgba(215,195,140,0.1)";
      ctx.textAlign="left";ctx.fillText("OUTPUT · Projects",45,587);ctx.restore();

      // ═══ SELF-ATTENTION ARCS ═══
      SELF_ATT.forEach(([a,b,w])=>{
        const na=nMap[a],nb=nMap[b];if(!na||!nb||!na.x||!nb.x)return;
        const aIn=aN.has(a)||a===act;const bIn=aN.has(b)||b===act;
        const both=ha&&aIn&&bIn;const oneIn=ha&&(aIn||bIn)&&!both;const dim=ha&&!both&&!oneIn;
        const cc=MC[na.mod]||MC.A;
        const sameCol=Math.abs(na.x-nb.x)<30;
        ctx.beginPath();
        if(sameCol){ctx.moveTo(na.x+4,na.y);ctx.quadraticCurveTo(na.x+18+w*12,(na.y+nb.y)/2,nb.x+4,nb.y)}
        else{const my=Math.min(na.y,nb.y)-8-w*6;ctx.moveTo(na.x,na.y);ctx.quadraticCurveTo((na.x+nb.x)/2,my,nb.x,nb.y)}
        if(dim){ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},0.003)`;ctx.lineWidth=0.05}
        else if(both){ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},${0.2+w*0.5})`;ctx.lineWidth=0.5+w*2.5}
        else if(oneIn){ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},${0.05+w*0.12})`;ctx.lineWidth=0.2+w*0.6}
        else{ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},${0.015+w*0.06})`;ctx.lineWidth=0.1+w*0.5}
        ctx.stroke();
      });

      // ═══ FLOW EDGES ═══
      E.forEach(([a,b])=>{
        const f=nMap[a],t=nMap[b];if(!f||!t||!f.x||!t.x)return;const ek=a+"→"+b;const ia=aE.has(ek);const sk=skipSet.has(ek);const cc=MC[f.mod]||MC.T;
        ctx.beginPath();
        if(sk){ctx.moveTo(f.x,f.y);ctx.quadraticCurveTo((f.x+t.x)/2,f.y<300?35:CH-20,t.x,t.y);ctx.setLineDash([3,3])}
        else{const dx=t.x-f.x,dy=t.y-f.y;ctx.moveTo(f.x,f.y);ctx.bezierCurveTo(f.x+dx*0.35,f.y,t.x-dx*0.35,t.y,t.x,t.y);ctx.setLineDash([])}
        if(ha){ctx.strokeStyle=ia?`rgba(${cc.r},${cc.g},${cc.b},${sk?0.4:0.15})`:"rgba(255,255,255,0.003)";ctx.lineWidth=ia?(sk?0.8:0.35):0.04}
        else{ctx.strokeStyle=sk?"rgba(210,190,140,0.05)":"rgba(255,255,255,0.018)";ctx.lineWidth=sk?0.4:0.18}
        ctx.stroke();ctx.setLineDash([]);
      });

      // Particles
      pts.current.forEach(p=>{p.t+=p.sp;if(p.t>=1){p.a=false;return}const t=p.t,mt=1-t,dx=p.tx-p.fx;const c1=p.fx+dx*0.35,c2=p.tx-dx*0.35;const px=mt*mt*mt*p.fx+3*mt*mt*t*c1+3*mt*t*t*c2+t*t*t*p.tx;const py=mt*mt*mt*p.fy+3*mt*mt*t*p.fy+3*mt*t*t*p.ty+t*t*t*p.ty;const al=Math.sin(t*Math.PI);ctx.beginPath();ctx.arc(px,py,p.sz+1.8,0,Math.PI*2);ctx.fillStyle=`rgba(210,195,130,${al*0.08})`;ctx.fill();ctx.beginPath();ctx.arc(px,py,p.sz,0,Math.PI*2);ctx.fillStyle=`rgba(240,225,165,${al*0.8})`;ctx.fill()});
      pts.current=pts.current.filter(p=>p.a);
      if(ha&&pts.current.length<60){aE.forEach(e=>{if(Math.random()<0.03){const[a,b]=e.split("→");const f=nMap[a],t=nMap[b];if(f&&t)pts.current.push({fx:f.x,fy:f.y,tx:t.x,ty:t.y,t:0,sp:0.002+Math.random()*0.003,sz:0.3+Math.random()*0.9,a:true})}})}

      // ═══ NODES ═══
      N.forEach(n=>{
        if(!n.x)return;
        const cc=MC[n.mod]||MC.T;const ia=ha&&(aN.has(n.id)||n.id===act);const ih=n.id===hov;
        const saW2=ha?saNeighbors[n.id]||0:0;const dim=ha&&!ia&&saW2===0;const isSA=ha&&!ia&&saW2>0;
        ctx.globalAlpha=dim?0.04:isSA?(0.15+saW2*0.55):1;
        const fa=ih?0.75:ia?0.5:isSA?(0.1+saW2*0.45):0.2;const sa=n.sch?SA_C[n.sch]:null;

        if(n.type==="repr"){
          rr(n.x-22,n.y-8,44,16,4);ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},${fa*0.8})`;ctx.fill();
          ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},${dim?0.04:0.3})`;ctx.lineWidth=0.4;ctx.stroke();
          for(let i=0;i<40;i+=2.5){const v=0.05+0.12*Math.sin(tm2.current*2+i*0.3+n.y*0.01);ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},${v})`;ctx.fillRect(n.x-20+i,n.y-6,1.5,12)}
          ctx.font="500 7px 'SF Mono','Menlo',monospace";ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},${dim?0.04:0.4})`;ctx.textAlign="center";ctx.fillText(n.l,n.x,n.y+20);
        }else if(n.type==="struct"){
          const tw=n.l.length*3.2+18,th=15;rr(n.x-tw/2,n.y-th/2,tw,th,3);
          ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},0.04)`;ctx.fill();
          ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},${dim?0.03:0.12})`;ctx.lineWidth=0.4;
          if(!n.id.includes("an"))ctx.setLineDash([2,2]);ctx.stroke();ctx.setLineDash([]);
          ctx.font="500 5.5px 'SF Mono','Menlo',monospace";ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},${dim?0.03:0.35})`;ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(n.l,n.x,n.y);
          if(n.id==="st-an"){ctx.font="500 10px monospace";ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},0.1)`;ctx.fillText("+",n.x+tw/2+8,n.y)}
        }else if(n.st===0){
          ctx.beginPath();ctx.arc(n.x,n.y,3,0,Math.PI*2);ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},${fa*1.2})`;ctx.fill();
          ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},${dim?0.04:0.35})`;ctx.lineWidth=0.25;ctx.stroke();
          if(sa){ctx.strokeStyle=`rgba(${sa.r},${sa.g},${sa.b},0.3)`;ctx.lineWidth=0.45;ctx.stroke()}
        }else if(n.st===1){
          rr(n.x-22,n.y-6,44,12,2);ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},${fa*0.65})`;ctx.fill();
          ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},${dim?0.04:0.18})`;ctx.lineWidth=0.25;ctx.stroke();
          if(sa){ctx.strokeStyle=`rgba(${sa.r},${sa.g},${sa.b},0.2)`;ctx.lineWidth=0.35;ctx.stroke()}
          if(ih||ia){ctx.font="500 4px 'SF Mono','Menlo',monospace";ctx.fillStyle=`rgba(255,255,255,${ih?0.8:0.5})`;ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(n.l,n.x,n.y)}
        }else if(n.st===3){
          rr(n.x-28,n.y-6,56,12,2);ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},${fa*0.55})`;ctx.fill();
          ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},${dim?0.03:0.14})`;ctx.lineWidth=0.25;ctx.stroke();
          if(sa){ctx.strokeStyle=`rgba(${sa.r},${sa.g},${sa.b},0.18)`;ctx.lineWidth=0.35;ctx.stroke()}
          ctx.font="500 3.5px 'SF Mono','Menlo',monospace";ctx.fillStyle=`rgba(255,255,255,${dim?0.03:(ih?0.7:0.28)})`;ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(n.l,n.x,n.y);
        }else if(n.st===4&&!n.type){
          rr(n.x-30,n.y-10,60,20,3);ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},${fa*0.55})`;ctx.fill();
          ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},${dim?0.03:0.12})`;ctx.lineWidth=0.35;ctx.stroke();
          if(sa){ctx.strokeStyle=`rgba(${sa.r},${sa.g},${sa.b},0.25)`;ctx.lineWidth=0.5;ctx.stroke()}
          ctx.font="500 4.5px 'SF Mono','Menlo',monospace";ctx.fillStyle=`rgba(255,255,255,${dim?0.03:(ih?0.8:0.45)})`;ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(n.l,n.x,n.y);
        }else if(n.st===5){
          ctx.beginPath();for(let i=0;i<6;i++){const ang=Math.PI/3*i-Math.PI/6;ctx.lineTo(n.x+10*Math.cos(ang),n.y+10*Math.sin(ang))}ctx.closePath();
          ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},${fa*1.1})`;ctx.fill();
          ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},${dim?0.04:0.3})`;ctx.lineWidth=0.35;ctx.stroke();
          if(sa){ctx.strokeStyle=`rgba(${sa.r},${sa.g},${sa.b},0.25)`;ctx.lineWidth=0.5;ctx.stroke()}
          ctx.font="500 5.5px 'SF Mono','Menlo',monospace";ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},${dim?0.03:0.55})`;ctx.textAlign="center";ctx.fillText(n.l,n.x,n.y-15);
          if(n.yr){ctx.font="400 4.5px 'SF Mono',monospace";ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},${dim?0.02:0.22})`;ctx.fillText(n.yr,n.x,n.y+18)}
        }else if(n.st===6){
          rr(n.x-38,n.y-5.5,76,11,2);ctx.fillStyle="rgba(255,255,255,0.01)";ctx.fill();
          ctx.strokeStyle=`rgba(${cc.r},${cc.g},${cc.b},${dim?0.02:0.07})`;ctx.lineWidth=0.2;ctx.stroke();
          ctx.font="500 4.5px 'SF Mono','Menlo',monospace";ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},${dim?0.03:0.32})`;ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(n.l,n.x,n.y);
        }
        if((ia||ih)&&!dim){ctx.beginPath();ctx.arc(n.x,n.y,15,0,Math.PI*2);ctx.fillStyle=`rgba(${cc.r},${cc.g},${cc.b},0.05)`;ctx.fill()}
        ctx.globalAlpha=1;
      });

      // Skip labels
      ctx.save();ctx.font="400 5px 'SF Mono',monospace";ctx.textAlign="center";
      ctx.fillStyle="rgba(200,170,120,0.08)";ctx.fillText("residual skip",1050,36);
      ctx.restore();

      // Footer
      ctx.save();ctx.font="400 5px 'SF Mono',monospace";ctx.fillStyle="rgba(255,255,255,0.06)";
      ctx.textAlign="left";ctx.fillText("Tsinghua · Cornell · Harvard GSD · MIT — 2020–2026",15,CH-6);
      ctx.textAlign="right";ctx.fillText("Multimodal Transformer · Time-Series Curriculum Architecture",CW-15,CH-6);
      ctx.restore();

      raf.current=requestAnimationFrame(draw);
    }
    draw();return()=>cancelAnimationFrame(raf.current);
  },[act,hov,dpr]);

  return React.createElement("div",{style:{position:"relative"}},
    React.createElement("canvas",{ref:cvs,onClick:oc,onMouseMove:om,onMouseLeave:()=>{setHov(null);setTip(null)},style:{width:"100%",minWidth:1000,height:"auto",aspectRatio:CW+"/"+CH,display:"block",borderRadius:8,cursor:"default"}}),
    tip&&React.createElement("div",{style:{position:"absolute",left:Math.min(tip.x,CW-160),top:Math.max(tip.y,0),background:"rgba(14,14,14,0.96)",border:"0.5px solid rgba(255,255,255,0.08)",borderRadius:4,padding:"5px 10px",pointerEvents:"none",zIndex:10,backdropFilter:"blur(12px)"}},
      React.createElement("div",{style:{color:"rgba(220,200,145,0.92)",fontSize:9,fontWeight:500}},tip.n.l),
      React.createElement("div",{style:{color:"rgba(255,255,255,0.28)",fontSize:7,marginTop:2}},(tip.n.sch||"")+(tip.n.sch?" · ":"")+(tip.n.yr?"Project "+tip.n.yr:tip.n.tm||"")+" · "+tip.n.mod)),
    React.createElement("div",{style:{display:"flex",gap:10,marginTop:10,flexWrap:"wrap",alignItems:"center"}},
      [{l:"THU",c:"rgba(255,255,255,0.18)"},{l:"Cornell",c:"rgba(180,40,40,0.5)"},{l:"Harvard",c:"rgba(200,170,100,0.5)"},{l:"MIT",c:"rgba(100,180,200,0.5)"}].map(s=>React.createElement("span",{key:s.l,style:{display:"flex",alignItems:"center",gap:3,fontSize:7,color:"rgba(255,255,255,0.18)"}},React.createElement("span",{style:{width:5,height:5,borderRadius:1,background:s.c,display:"inline-block"}}),s.l)),
      React.createElement("span",{style:{marginLeft:"auto",fontSize:5.5,color:"rgba(255,255,255,0.1)"}},"○ token · ▭ enc · ▬ repr · ⌒ self-attn · ⬡ output · ╌ skip"))
  );
}

window.NNSelfAttn = NNSelfAttn;
