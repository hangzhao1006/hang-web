...
(function(){
  ...
  sk.forEach((s,i)=>{s.x=SX[6];s.y=90+i*(CH-150)/(sk.length-1)});
...
function NNSelfAttn(){
  ...
  useEffect(()=>{
    ...
    function draw(){
      ...
      Object.values(SX).filter(x=>x>0).forEach(x=>{ctx.beginPath();ctx.moveTo(x,80);ctx.lineTo(x,660);ctx.stroke()});
      ctx.restore();
      ...
      ctx.fillText("residual skip (h_A)",830,95);ctx.fillStyle=TH.skipC;ctx.fillText("residual skip (h_C)",830,620);ctx.restore();
