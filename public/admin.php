  ...
  if ($is_logged_in && in_array($act, ['create', 'update', 'delete'], true)) {
    ...
    function collect_meta_data() {
        ...
        foreach ($basic_keys as $k) {
            if (!empty($_POST["meta_{$k}"])) $meta[$k] = trim($_POST["meta_{$k}"]);
        }
...
<head>
  ...
  <style>
    /* Editors Tweak */
    .EasyMDEContainer .CodeMirror { border-color: #ddd; border-radius: 4px; font-family: monospace; font-size: 14px; }
    .editor-toolbar { border-color: #ddd; opacity: 0.6; transition: opacity 0.2s; }
    .editor-toolbar:hover { opacity: 1; }
    
    /* Block Builder Styles */
    .blocks-container { display: flex; flex-direction: column; gap: 15px; margin-bottom: 20px; min-height: 50px; padding: 5px; }
    .content-block-item { 
        background: #fff; border: 1px solid #e0e0e0; border-radius: 8px; padding: 15px; 
        position: relative; display: flex; flex-direction: column; gap: 12px;
        box-shadow: 0 2px 5px rgba(0,0,0,0.02);
    }
    .content-block-item.sortable-ghost { opacity: 0.4; border: 2px dashed #999; background: #f0f0f0; }
    .content-block-item.sortable-drag { box-shadow: 0 10px 20px rgba(0,0,0,0.15); transform: scale(1.02); cursor: grabbing; }
    
    .block-header { 
        display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px; 
        padding-bottom: 10px; border-bottom: 1px solid #f0f0f0; cursor: grab;
    }
    .block-handle { color: #ccc; margin-right: 10px; font-size: 1.2rem; user-select: none; }
    .block-type { font-size: 0.75rem; text-transform: uppercase; font-weight: bold; color: #666; background: #eee; padding: 3px 8px; border-radius: 4px; }
    ...
    .range-header {
        display: flex; justify-content: space-between; font-size: 0.8rem; color: #666; font-weight: 500;
    }
    .range-val { color: #111; font-family: monospace; background: #eee; padding: 2px 6px; border-radius: 4px; }
    
    input[type=range] {
        -webkit-appearance: none; width: 100%; background: transparent;
    }
    input[type=range]::-webkit-slider-thumb {
        -webkit-appearance: none; height: 16px; width: 16px; border-radius: 50%; background: #111; cursor: pointer; margin-top: -6px;
    }
    ...
    .visual-controls {
        background: #f9f9f9; padding: 15px; border-radius: 6px; border: 1px solid #eee; display: flex; gap: 20px;
    }
    .visual-col { flex: 1; }

    .add-block-bar { display: flex; gap: 10px; margin-top: 20px; border-top: 1px dashed #ddd; padding-top: 20px; }
    .add-block-btn { flex: 1; padding: 12px; background: #fff; border: 1px dashed #ccc; color: #666; cursor: pointer; border-radius: 6px; transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 8px; }
    .add-block-btn:hover { border-color: #333; color: #333; background: #fdfdfd; box-shadow: 0 2px 8px rgba(0,0,0,0.05); }
  </style>
</head>
          ...
          <div class="editor-card collapsed" id="project-<?= $p['id'] ?>">
            ...
            <div class="card-header" onclick="toggleEdit(<?= $p['id'] ?>)">
                <div class="header-info">
                    ...
                    <span class="status-badge <?= $p['featured'] ? 'featured' : '' ?>"><?= $p['featured'] ? 'Featured' : 'Standard' ?></span>
                </div>
            ...
            <div class="card-body">
                <form method="post" enctype="multipart/form-data" class="project-form" data-id="<?= $p['id'] ?>">
                    ...
                    <div class="tabs-nav">
                        <button type="button" class="tab-btn active" onclick="switchTab(<?= $p['id'] ?>, 'basic')">Basic Info</button>
                        <button type="button" class="tab-btn" onclick="switchTab(<?= $p['id'] ?>, 'story')">Story Builder</button>
                        <button type="button" class="tab-btn" onclick="switchTab(<?= $p['id'] ?>, 'media')">Media & Visuals</button>
                    </div>

                    <!-- Tab 1: Basic -->
                    <div class="tab-pane active" data-tab="basic-<?= $p['id'] ?>">
                        <div class="form-grid">
                            <div class="form-group full"><label>Title</label><input name="title" value="<?= htmlspecialchars($p['title']) ?>"></div>
                            <div class="form-group full"><label>Subtitle</label><input name="meta_subtitle" value="<?= htmlspecialchars($meta['subtitle'] ?? '') ?>"></div>
                            <div class="form-group"><label>Slug</label><input name="slug" value="<?= htmlspecialchars($p['slug']) ?>"></div>
                            <div class="form-group"><label>Year</label><input name="year" type="number" value="<?= $p['year'] ?>"></div>
                            <div class="form-group"><label>Role</label><input name="meta_role" value="<?= htmlspecialchars($meta['role'] ?? '') ?>"></div>
                            <div class="form-group"><label>Client</label><input name="meta_client" value="<?= htmlspecialchars($meta['client'] ?? '') ?>"></div>
                            <div class="form-group full"><label>Tags</label><input name="tags" value="<?= htmlspecialchars($p['tags']) ?>"></div>
                            <div class="form-group full"><label class="checkbox-label"><input type="checkbox" name="featured" value="1" <?= $p['featured'] ? 'checked' : '' ?>> Featured</label></div>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane" data-tab="story-<?= $p['id'] ?>">
                        <div class="form-group">
                            ...
                            <textarea name="description" class="simple-editor" rows="3"><?= htmlspecialchars($p['description']) ?></textarea>
                        </div>
                        
                        <div class="divider"><span>Content Blocks (Drag to Reorder)</span></div>
                        
                        <div class="blocks-container" id="blocks-container-<?= $p['id'] ?>"></div>
                        <input type="hidden" name="content_blocks_json" id="blocks-input-<?= $p['id'] ?>" value="<?= $blocksJson ?>">

                        <div class="add-block-bar">
                            <button type="button" class="add-block-btn" onclick="addBlock(<?= $p['id'] ?>, 'text')"><span class="icon">T</span> Text</button>
                            <button type="button" class="add-block-btn" onclick="addBlock(<?= $p['id'] ?>, 'image')"><span class="icon">🖼️</span> Image</button>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane" data-tab="media-<?= $p['id'] ?>">
                        <div class="form-group">
                            ...
                            <div class="file-input-wrapper"><small>Or upload:</small><input type="file" name="cover" accept="image/*"></div>
                        </div>
                        ...
                        <div class="visual-controls">
                            ...
                            <div class="visual-col">
                                <div class="range-control-group">
                                    <div class="range-header"><span>Hero Height</span><span class="range-val" id="val-height-<?= $p['id'] ?>"><?= $heroHeight ?>vh</span></div>
                            ...
                            <div class="visual-col">
                                <div class="range-control-group">
                                    <div class="range-header"><span>Focus Y (Top -> Bottom)</span><span class="range-val" id="val-pos-<?= $p['id'] ?>"><?= $heroPosY ?>%</span></div>
                            ...
                            <div class="visual-col">
                                <div class="range-control-group">
                                    <div class="range-header"><span>Scale (Zoom)</span><span class="range-val" id="val-scale-<?= $p['id'] ?>"><?= $heroScale ?>x</span></div>
                        ...
                        <div class="form-group">
                            <textarea name="meta_links_json" rows="3" class="code-font"><?= htmlspecialchars($linksJson) ?></textarea>
                        </div>
                    </div>

                    <div class="card-actions">
                        <button type="button" class="btn-primary" onclick="submitProjectForm(<?= $p['id'] ?>)">Save Changes</button>
                        <button type="submit" formaction="?action=delete" name="action" value="delete" class="btn-danger" onclick="return confirm('Really delete?');">Delete</button>
                    </div>
                </form>
            </div>
          </div>
